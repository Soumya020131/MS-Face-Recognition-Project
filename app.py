...
ALLOWED_EXTENSIONS_VID = set(['txt','mp4'])
ALLOWED_EXTENSIONS_IMG = set(['png','jpg','jpeg'])
CRIMINAL_UPLOAD_FOLDER = 'static/Criminals/'
MISSING_UPLOAD_FOLDER = 'static/Missing/'
app.config['CRIMINAL_UPLOAD_FOLDER'] = CRIMINAL_UPLOAD_FOLDER
app.config['MISSING_UPLOAD_FOLDER'] = MISSING_UPLOAD_FOLDER
TEST_UPLOAD_FOLDER = 'static/Test/'
...
def get_criminal():
    return update_dataset('CRIMINAL_UPLOAD_FOLDER')
    
    
@app.route('/missing_rec', methods=['GET', 'POST'])
def get_missing():
    return update_dataset('MISSING_UPLOAD_FOLDER')
...
def delete_missing():
    return delete_file(MISSING_UPLOAD_FOLDER)

@app.route('/deletefile_from_criminal_rec', methods=['GET', 'POST'])
def delete_criminal():
    return delete_file(CRIMINAL_UPLOAD_FOLDER)
...
def get_test():
    ...
    if request.method == 'POST':
        ...
        if pic and allowed_file(pic.filename):
            filename = secure_filename(pic.filename)
            pic.save(os.path.join(app.config['TEST_UPLOAD_FOLDER'], file_name))       
